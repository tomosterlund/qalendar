...
describe('EventConcurrencyHelper.ts', () => {
	...
	test('passing in two concurrent events', () => {
		const unprocessedEvents: eventInterface[] = [
			...
			},
			{
				...
				description: 'Introduction to linear algebra. Please bring textbook and exercise book 7A'
			},
		]

		const concurrentEvents = eventConcurrencyHelper.calculateConcurrencyForEvents(unprocessedEvents)
	...
	test('one long event with two periods of concurrent events', () => {
		...
		const concurrentEvents = eventConcurrencyHelper.calculateConcurrencyForEvents(events)
