...
describe('DayEvent.vue', () => {
	...
	test('Setting colors via a colorScheme', async () => {
		...
		await nextTick()
		expect(eventElement.attributes().style).toContain('background-color: rgb(255, 64, 129)') // The pink BG
		expect(eventElement.attributes().style).toContain('color: rgb(255, 255, 255)') // The white text
	})
})