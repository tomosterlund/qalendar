	...
	calculateConcurrencyForEvents(events: eventInterface[]) {
		...
		for (const [index, calendarEvent] of updatedEvents.entries()) {
			...
			while (iterator < index) {
				if (updatedEvents[iterator].time.end > calendarEvent.time.start) nOfConcurrentEvents++
				iterator++
			}

			if (nOfConcurrentEvents) updatedEvents[index].nOfPreviousConcurrentEvents = nOfConcurrentEvents
		}
		...
		for (let index = updatedEvents.length - 1; index >= 0; index--) {
			...
			updatedEvents[index].totalConcurrentEvents = nfOfPreviousEvents + nOfUpcomingConcurrentEvents + 1 // One for the event itself
		}
